...
**For the eventual V4 → main merge:** main has one commit V4 lacks, `fb92bba` (Liann's second
implementation of channel selection; V4 has its own, `ebb2b86` and follow-ups). A trial merge on
2026-09-24 conflicted in 7 files (`RunStart.tsx`, `RunStart.test.tsx`, `feasibility.py`, `handoff.py`,
`models.py`, `prompts.py`, `test_handoff.py`). Keeping V4's side passed 805/806 backend tests; the one
failure is main's test asserting main's call-directive wording — update it to V4's wording.
