...
**Tech Stack:** Python 3.12, FastAPI, Pydantic 2, SQLAlchemy 2, Alembic, asyncpg, httpx, Anthropic SDK, pytest, Next.js with TypeScript, React, Vitest, Testing Library, Playwright, Postgres/Supabase, Railway, Vercel.
...
```bash
npx create-next-app@latest apps/web --ts --eslint --app --src-dir --use-npm --no-tailwind --import-alias '@/*'
uv init --python 3.12 --package services/api
cd services/api
...
npx openapi-typescript ../../contracts/openapi.json -o src/lib/api-types.ts
```
...
jobs:
  ...
  web:
    ...
    steps:
      ...
      - run: cd apps/web && npm ci && npm test -- --run && npm run lint && npm run build
```
