...
async def seed_ready_winner(db_session: AsyncSession, *, audience_query: str) -> None:
    db_session.add(RunRow(
        id="run-t", pro_ids=["pro_1"], audience_query=audience_query,
        audience_run="2026-08-06T18:00:00Z", channels=["sms"],
        cost_limit=Decimal("100.00"),
    ))
    ...
    db_session.add(WinnerRow(
        ...
        candidate_id="cand-1", evidence={"org_id": "882486"},
    ))
...
async def test_trickle_push_sends_ready_winner_and_is_idempotent(
    ...
    assert await push_ready_winners(db_session, STUB_SETTINGS, "run-t") == 1
    # Second call (the next Pro finishing) finds nothing unanswered: no POST.
    assert await push_ready_winners(db_session, STUB_SETTINGS, "run-t") == 0
    assert len(httpx_mock.get_requests()) == 1
